"""
منصة الحبي للتداول — Pro v6.1
تصميم احترافي + Lightweight Charts v5 + إصلاح خطأ السحابة
"""
...
.logo{{width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,var(--acc),#7C3AED);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:20px}}
...
def analyze(ticker, smt):
    try:
        ...
        r["_cur"]=None; r["_df"]=pd.DataFrame(); r["_setup"]=None
        return r
...
def market_status(mkt):
    ...
    now = datetime.now(tz)
    if mkt=="SA":
        open_m = now.weekday() in [6,0,1,2,3] and 10 <= now.hour < 15
    else:
        open_m = now.weekday()<5 and (9,30) <= (now.hour,now.minute) < (16,0)
    return open_m

# ========== HEADER ==========
open_us = market_status("US")
open_sa = market_status("SA")
st.markdown(f"""
<div class="header">
  <div style="display:flex;gap:12px;align-items:center">
    <div class="logo">ح</div>
    <div><div class="brand">منصة الحبي Pro</div><div style="font-size:.75rem;color:var(--txt2)">تداول ذكي • تحليل لحظي</div></div>
  </div>
  <div style="display:flex;gap:8px">
    <div class="tab {'active' if st.session_state.market=='SA' else ''}">🇸🇦 {'🟢' if open_sa else '🔴'}</div>
    <div class="tab {'active' if st.session_state.market=='US' else ''}">🇺🇸 {'🟢' if open_us else '🔴'}</div>
  </div>
</div>
""", unsafe_allow_html=True)
...
scan = col3.button("📡 مسح", type="primary", use_container_width=True)
    ...
    for _,r in df.head(24).iterrows():
        ...
        <div class="card">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:.9rem">
            ...
            <div>حالي<br><b>{cur_display}</b></div>
          </div>
    ...
    if pick:
        ...
        if setup is not None and not df_d.empty:
            ...
            charts = [{
                ...
                "series": [{"type":"Candlestick","data":candles,"options":{"upColor":GR,"downColor":RD}}],
                "priceLines": [
            ...
            lightweight_charts_v5_component(name=f"{pick}", charts=charts, height=440)
else:
